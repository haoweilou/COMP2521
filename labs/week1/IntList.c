...
void IntListInsertInOrder (IntList L, int v)
{
    ...
    struct IntListNode *pre = NULL;
    struct IntListNode *nex = NULL;
    //if there is no node in the link list, make first and last be the frist
    //entered list
    ...
    for(struct IntListNode *cur = L->first;cur != NULL; cur = cur->next){
        nex = cur->next;
        if(v<= L->first->data) {
            L->first = new;
            new->next = cur;
