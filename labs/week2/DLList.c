...
// Modified by Haowei Lou, June 2019
...
{
    //if no node in L, make the new be first,curr and last
	...
	DLListNode *prev = L->curr->prev;
	new->next = curr;
	curr->prev = new;
	new->prev = prev;
	L->first = (prev == NULL) ? new : L->first;
	if(prev != NULL) {
    	prev->next = new;
	}
	L->curr = new;
...
void DLListAfter (DLList L, char *it)
{
	...
	assert (L != NULL);
	DLListNode *curr = L->curr;
	DLListNode *next = L->curr->next;
	
	curr->next = new;
	new->next = next;
	new->prev = curr;
	L->last = (next == NULL) ? new : L->last;
	if(next != NULL) {
    	next->prev = new;
	}
	L->curr = new;
...
void DLListDelete (DLList L)
{
	...
	DLListNode *prev = L->curr->prev;
	freeDLListNode (curr);
	//release all node in the L if it only contain one node
	if(L->last == L->first){
    	L->first = NULL;
    	L->last = NULL;
    	L->curr =NULL;
	...
	//make the node after first become new *first node
	if(curr == L->first) {
	    L->first = next;
	    next->prev = NULL;
	    L->curr = next;
