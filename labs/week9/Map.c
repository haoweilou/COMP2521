...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
	...
	for (VList n = g->connections[start]; n != NULL; n = n->next)
	{
	    if(n->v == end)
	    {
	        switch(n->type)
            {
                case ROAD: type[i] = n->type;
	            case RAIL: type[i] = n->type;
	            case BOAT: type[i] = n->type;
	            case NONE: ;
	            case ANY: ;
            }
            i++;   
	...
	for(int j = 0; j < i; j++)
	{
		if(type[j] == BOAT) return i;
	}
	for (VList n = g->connections[start]; n != NULL; n = n->next)
	{
		for (VList m = g->connections[end]; m != NULL; m = m->next)
		{
			if(n->v == m->v && idToType(n->v) == SEA){
				type[i] = BOAT;
				i++;
				return i;
			}
